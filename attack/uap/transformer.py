...
# def save_tensor_to_txt(tensor: torch.Tensor, filename: str, float_format: str = ".4f"):
#     shape_info = tensor.shape
#     array = tensor.detach().cpu().numpy()
#
#     # 处理不同维度格式
#     with open(filename, 'w') as f:
#         f.write(str(shape_info))
#         if array.ndim == 1:
#             line = ' '.join(format(x, float_format) for x in array)
#             f.write(line + '\n')
#         elif array.ndim == 2:
#             for row in array:
#                 line = ' '.join(format(x, float_format) for x in row)
#                 f.write(line + '\n')
#         else:
#             # 对于高维张量，写成分块形式
#             for i, subarray in enumerate(array):
#                 f.write(f"# Slice {i}:\n")
#                 np.savetxt(f, subarray, fmt=f"%{float_format}")
#                 f.write("\n")
#     print("Saved tensor to txt at: {}".format(filename))
#
#
...
class PatchTransformer(nn.Module):
    def __init__(self, device: torch.device, cfg_patch: object,
        ...
        The settings of random jitter, rotation, shift(not used in T-SEA) and scale 
        follow AdvPatch (http://openaccess.thecvf.com/content_CVPRW_2019/papers/CV-COPS/Thys_Fooling_Automated_Surveillance_Cameras_Adversarial_Patches_to_Attack_Person_Detection_CVPRW_2019_paper.pdf).
    ...
    def forward(self, adv_patch_batch, bboxes_batch, patch_ori_size,
        ...
        # save_tensor_to_txt(tensor=bboxes_batch, filename="temp/bboxes_batch_yolov5.txt")
        # save_tensor_to_txt(tensor=bboxes_batch, filename="temp/bboxes_batch_defDETR.txt")
