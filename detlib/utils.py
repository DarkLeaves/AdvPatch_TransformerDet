...
def init_detector(detector_name: str,
                  cfg: object,
    ...
    if detector_name == 'def_detr':
        from detlib.TransformerDet import DeformableDETRDetector
        detector = DeformableDETRDetector(name=detector_name, cfg=cfg, input_tensor_size=0, device=device)
        detector.load()
