...
def obj_tv_fi_loss(**kwargs):
    confs = kwargs['confs']
    patch = kwargs['patch']
    # weights = kwargs['weights']; feature_map = kwargs['feature_map']
    extr_loss = kwargs['extr_loss']
    tv_loss = TVLoss.smooth(patch)
    obj_loss = torch.mean(confs)
    # assert weights.shape == feature_map.shape
    # cwh = weights.shape[0] * weights.shape[1] * weights.shape[2] * weights.shape[3]
    # fi_loss = torch.sum(weights * feature_map) / cwh
    loss = {'tv_loss': tv_loss, 'obj_loss': obj_loss, 'extr_loss': extr_loss}
    return loss
