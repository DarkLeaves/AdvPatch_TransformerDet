...
from attack.attacker import TransUniversalAttacker
from utils.parser import ConfigParser
import warnings
warnings.filterwarnings("ignore", module="OpenBLAS")

# use hugging-face mirror site for network connection
os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"



def init(detector_attacker: TransUniversalAttacker, cfg: ConfigParser, data_root: str, args: object =None, log: bool =True):
...
def train_uap(cfg: ConfigParser,
              detector_attacker: TransUniversalAttacker,
              save_name: str,
              args: object =None,
              data_root: str =None):
    ...
    for epoch in range(args.start_epoch, cfg.ATTACKER.MAX_EPOCH + 1):
        ...
        if epoch % 10 == 0 or epoch == cfg.ATTACKER.MAX_EPOCH:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-cfg', '--cfg', type=str, default='Transformers/def_detr.yaml', help="A relative path of the .yaml proj config file.")
    parser.add_argument('-n', '--board_name', type=str, default='d_detr_Lobj', help="Name of the Tensorboard as well as the patch name.")
    parser.add_argument('-d', '--debugging', action='store_true', help="Will not start tensorboard process if debugging=True.")
    parser.add_argument('-s', '--save_path', type=str, default='./results/att_trans/def_detr', help="Path to save the adversarial patch.")
    parser.add_argument('-np', '--new_process', action='store_true', default=True, help="Start new TensorBoard server process.")
    ...
    detector_attacker = TransUniversalAttacker(cfg, device)
