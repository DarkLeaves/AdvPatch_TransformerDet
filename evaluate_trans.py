...
from attack.attacker import UniversalAttacker, TransUniversalAttacker
...
class TransPatchEvaluator(TransUniversalAttacker):
    def __init__(self, cfg, patch_path=None,
                 device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"),
                 ):
        super().__init__(cfg, device)
...
def eval_init(args, cfg, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")):
    ...
    evaluator = TransPatchEvaluator(cfg, args.patch, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--patch', type=str, default='./results/att_trans/def_detr/d_detr_Lobj_30.png', help="Optional. Init by an given patch.")
    parser.add_argument('-cfg', '--cfg', type=str, default='./configs/eval/coco80_trans.yaml', help="A RELATIVE file path of teh .yaml cfg file.")
    parser.add_argument('-s', '--save', type=str, default='./data/attack_result/d_detr', help="Directory to save evaluation results. ** Use an ABSOLUTE path.")
